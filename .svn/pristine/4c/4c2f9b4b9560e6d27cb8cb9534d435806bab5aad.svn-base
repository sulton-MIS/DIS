...
EXEC SP_INSERT_UPDATE_LOG_H @USERNAME,'','WP01007',1,'1','CREATE',@@COUNTER;
...
BEGIN TRY
	SET @@CNT = (SELECT COUNT(1) FROM TB_M_ITEM WHERE ITEM_NAME = @ITEM_NAME);
	...
	BEGIN
		...
		DECLARE @@MSG_WARNING VARCHAR(MAX) = 'MWP004 : '+ (SELECT dbo.GenerateMessage('MWP004','ITEM','',''));
		EXEC SP_INSERT_LOG_D @@COUNTER, 'Duplicate', 'MWP004', 'Info',  @@MSG_WARNING;
		
	END ELSE
	BEGIN
		
		INSERT INTO TB_M_ITEM
		(
			 [ITEM_NAME]
			,[ITEM_TYPE]
			,[ITEM_ST]
		...
		DECLARE @@MSG_SUCCESS VARCHAR(MAX) = 'MWP001 : '+ (SELECT dbo.GenerateMessage('MWP001','ITEM MASTER','',''));
...
BEGIN CATCH
	...
	SET @@CHK = 'FALSE';
	SET @@ERR = 'ERROR INSERT TB_M_ITEM:' +@ITEM_NAME+
