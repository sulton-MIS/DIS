...
EXEC SP_INSERT_UPDATE_LOG_H @USERNAME,'','WP01002',1,'1','CREATE',@@COUNTER;
...
BEGIN TRY
	SET @@CNT = (SELECT COUNT(1) FROM TB_M_AREA WHERE AREA_CD = @AREA_CODE);
	...
	BEGIN
		...
		DECLARE @@MSG_WARNING VARCHAR(MAX) = 'MWP004 : '+ (SELECT dbo.GenerateMessage('MWP004','AREA CODE','',''));
	...
	BEGIN
		...
		(
			...
			@AREA_NAME,
			@AREA_STATUS,
			'Content/Upload/Area/'+REPLACE(@AREA_CODE+'.'+@EXT, ' ', ''),
			@USERNAME,
			GETDATE(),
			@USERNAME,
			GETDATE(),
		...
		DECLARE @@MSG_SUCCESS VARCHAR(MAX) = 'MWP001 : '+ (SELECT dbo.GenerateMessage('MWP001','AREA MASTER','',''));
...
BEGIN CATCH
	...
	SET @@CHK = 'FALSE';
	SET @@ERR = 'ERROR INSERT TB_M_AREA: ' +@AREA_CODE+
