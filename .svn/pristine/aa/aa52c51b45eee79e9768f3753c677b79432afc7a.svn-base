...
BEGIN TRY
	SET @@CNT = (SELECT COUNT(1) FROM TB_R_WP_DETAIL_HENKATEN_ENV WHERE ID = @ID);
	IF(@@CNT > 0)
	BEGIN
		...
		UPDATE TB_R_WP_DETAIL_HENKATEN_ENV 
			SET [IS_HENKATEN_ENV] = @IS_HENKATEN_ENV
			  ,[HK_ENV_PROCESS] = @HK_ENV_PROCESS
			  ,[HK_ENV_MACHINE] = @HK_ENV_MACHINE
			  ,[HK_ENV_MATERIAL] = @HK_ENV_MATERIAL
			  ,[HK_ENV_BUILDING] = @HK_ENV_BUILDING
			  ,[CHANGED_BY] = @username
			  ,[CHANGED_DT] = GETDATE()
		WHERE ID = @ID;

		SET @@MSG_SUCCESS  = 'MWP003 : '+ (SELECT dbo.GenerateMessage('MWP003','HENKATEN ENV','',''));
	...
	BEGIN
		...
		INSERT INTO TB_R_WP_DETAIL_HENKATEN_ENV
		([WP_PROJECT_JOB_ID]
           ,[IS_HENKATEN_ENV]
           ,[HK_ENV_PROCESS]
           ,[HK_ENV_MACHINE]
		...
		SET @@MSG_SUCCESS = 'MWP001 : '+ (SELECT dbo.GenerateMessage('MWP001','HENKATEN ENV','',''));
...
BEGIN CATCH
	SET @@CHK = 'FALSE';
	SET @@ERR = 'ERROR INSERT TB_R_WP_DETAIL_HENKATEN_ENV:' +@WP_PROJECT_JOB_ID+
