...
EXEC SP_INSERT_UPDATE_LOG_H @USERNAME,'','WP01007',1,'1','DELETE',@@COUNTER;
...
IF NOT EXISTS (SELECT 1
					FROM TB_R_WP_DAILY_WORK_EQUIPMENT WHERE TB_M_ITEM_ID = @ID)
BEGIN
	DELETE TB_M_ITEM WHERE ID_TB_M_ITEM = @ID;
	SELECT 'True' AS MSG;
	DECLARE @@MSG_SUCCESS VARCHAR(MAX) = 'MWP002 : '+ (SELECT dbo.GenerateMessage('MWP002','ITEM','',''));
	EXEC SP_INSERT_LOG_D @@COUNTER, 'Deleted', 'MWP002', 'Info',  @@MSG_SUCCESS;
END
ELSE
BEGIN
	SELECT 'False' AS MSG;
	DECLARE @@MSG_WARNING VARCHAR(MAX) = 'MWP009 : '+ (SELECT dbo.GenerateMessage('MWP009','ITEM','',''));
...
@USERNAME,'','WP01007','1',1,'DELETE',@@COUNTER;