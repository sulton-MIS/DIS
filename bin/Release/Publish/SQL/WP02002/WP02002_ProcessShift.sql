...
BEGIN TRY
	...
	BEGIN
		...
		UPDATE [dbo].[TB_R_WP_DETAIL_SHIFT]
		SET [SHIFT_ID] = @SHIFT_ID
			,[CHANGED_BY] = @username
			,[CHANGED_DT] = GETDATE()
		WHERE ID = @ID;

		SET @@MSG_SUCCESS  = 'MWP003 : '+ (SELECT dbo.GenerateMessage('MWP003','IMPB SHIFT','',''));
	...
	BEGIN
		...
		INSERT INTO [dbo].[TB_R_WP_DETAIL_SHIFT]
           ([WP_PROJECT_JOB_ID]
           ,[SHIFT_ID]
           ,[CREATED_BY]
           ,[CREATED_DT]
		...
		SET @@MSG_SUCCESS = 'MWP001 : '+ (SELECT dbo.GenerateMessage('MWP001','IMPB SHIFT','',''));
...
BEGIN CATCH
	SET @@CHK = 'FALSE';
	SET @@ERR = 'ERROR INSERT SHIFT:' +@SHIFT_ID+
