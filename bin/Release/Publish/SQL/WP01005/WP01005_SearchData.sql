...
IF(@DEPARTEMENT <> '')
	BEGIN
		SET @@QUERY = @@QUERY + 'AND dept LIKE ''%'+RTRIM(@DEPARTEMENT)+'%'' ';
	END

IF(@DIVISION <> '')
	BEGIN
		SET @@QUERY = @@QUERY + 'AND bagian LIKE ''%'+RTRIM(@DIVISION)+'%'' ';
	END

IF(@POSITION <> '')
	BEGIN
		SET @@QUERY = @@QUERY + 'AND jabatan LIKE ''%'+RTRIM(@POSITION)+'%'' ';
	END

IF(@GROUP <> '')
	BEGIN
		SET @@QUERY = @@QUERY + 'AND grp LIKE '''+(@GROUP)+''' ';
	END
