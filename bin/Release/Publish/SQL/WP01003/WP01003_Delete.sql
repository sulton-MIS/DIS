...
EXEC SP_INSERT_UPDATE_LOG_H @USERNAME,'','WP01003',1,'1','DELETE',@@COUNTER;
...
IF NOT EXISTS (SELECT 1
					FROM TB_R_WP_PROJECT AS A INNER JOIN TB_M_LOCATION AS B ON A.ID_TB_M_AREA = B.ID_TB_M_AREA
					WHERE B.ID_TB_M_LOCATION = @ID)
BEGIN
	DELETE TB_M_LOCATION WHERE ID_TB_M_LOCATION = @ID;
	SELECT 'True' AS MSG;
	DECLARE @@MSG_SUCCESS VARCHAR(MAX) = 'MWP002 : '+ (SELECT dbo.GenerateMessage('MWP002','LOCATION','',''));
	EXEC SP_INSERT_LOG_D @@COUNTER, 'Deleted', 'MWP002', 'Info',  @@MSG_SUCCESS;
END
ELSE
BEGIN
	SELECT 'False' AS MSG;
	DECLARE @@MSG_WARNING VARCHAR(MAX) = 'MWP009 : '+ (SELECT dbo.GenerateMessage('MWP009','LOCATION','',''));
...
@USERNAME,'','WP01003','1',1,'DELETE',@@COUNTER;