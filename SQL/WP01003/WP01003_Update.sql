...
EXEC SP_INSERT_UPDATE_LOG_H @USERNAME,'','WP01003',1,'1','Update',@@COUNTER;
...
BEGIN TRY
	UPDATE TB_M_LOCATION SET ID_TB_M_AREA = @AREA_CODE, LOC_CD = @LOC_CD, LOC_NAME = @LOC_NAME, STATUS = @STATUS, UPDATE_BY = @USERNAME, UPDATE_DT = GETDATE() WHERE ID_TB_M_LOCATION = @ID
	
	DECLARE @@MSG_SUCCESS VARCHAR(MAX) = 'MWP003 : '+ (SELECT dbo.GenerateMessage('MWP003','LOCATION','',''));
