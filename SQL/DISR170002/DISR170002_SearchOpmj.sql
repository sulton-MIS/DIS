...
SET @@QUERY = '
SELECT 
	ROW_NUMBER() OVER (ORDER BY [SC].DMC_CODE ASC) ROW_NUM,
	[SC].DMC_CODE as ID,
	[SC].DMC_CODE as DMC_CODE,	
...
FROM (
		SELECT 	 
			dbo.Z_RT_data_J_kotei.id_seihin AS DMC_CODE,			
...
SET @@QUERY = @@QUERY + '						 
		...
		GROUP BY 	
			shift_date, Z_RT_data_J_kotei.id_seihin
...
IF(@DMC_CODE <> '')
BEGIN
	SET @@QUERY = @@QUERY + 'AND [SC].DMC_CODE LIKE ''%'+@DMC_CODE+'%'' ';
END

SET @@QUERY = @@QUERY + 'GROUP BY [SC].DMC_CODE' ;
