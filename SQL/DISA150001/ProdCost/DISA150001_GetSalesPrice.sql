...
LEFT OUTER JOIN 
		(
			SELECT
				item_code,
				MAX(AIR_JPN) as AIR_JPN,
				MAX(SEA_TOKYO) as SEA_TOKYO,
				MAX(AIR_SHA) as AIR_SHA,
				MAX(SEA_SHA) as SEA_SHA,
				MAX(AIR_HKG) as AIR_HKG,
				MAX(SEA_HKG) as SEA_HKG
			FROM
			(
				SELECT	
					item_code,
					case when TR.code_trans LIKE '%TACJ001%' then TR.total_cost else 0 end as AIR_JPN,					
					case when TR.code_trans LIKE '%TSCJ002%' then TR.total_cost else 0 end as SEA_TOKYO,
					case when TR.code_trans LIKE '%TACS003%' then TR.total_cost else 0 end as AIR_SHA,
					case when TR.code_trans LIKE '%TSCS004%' then TR.total_cost else 0 end as SEA_SHA,
					case when TR.code_trans LIKE '%TACH005%' then TR.total_cost else 0 end as AIR_HKG,
					case when TR.code_trans LIKE '%TSCH006%' then TR.total_cost else 0 end as SEA_HKG
				FROM
					ad_dis_pc_transportation_sum TR
				WHERE 
					item_code = @DMC_CODE
			) as Trans
			GROUP BY item_code
