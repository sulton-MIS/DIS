...
--======================================================
--SAMPLE QUERY WHEN RUN SSMS MODIFIKASI FORMULA 20220816
--======================================================
...
--				Z_RT_data_J_kotei.shift_date AS SHIFT_DATE,
--				SUM(amnt_OK) + SUM(amnt_NG) + SUM(amnt_PND) AS QTY_INPUT,
--				Z_PRTS.SIYOUW AS CAVITY,
--				(SUM(amnt_OK) + SUM(amnt_NG) + SUM(amnt_PND)) * Z_PRTS.SIYOUW AS QTY_INPUT_PCS,
--				((SUM(ttl_sagyo)) - (ISNULL(SUM(J_PAUSE_WORKING.ttl_pause_working),0)) - (ISNULL(SUM(J_SETTING_AFTER.ttl_setting_after), 0))) AS WORKING_TIME
--			FROM 
--				dbo.Z_RT_data_J_kotei 
--				INNER JOIN 
...
--				ON Z_RT_data_J_kotei.id_seisan = J_SETTING_AFTER.id_seisan AND Z_RT_data_J_kotei.id_kotei = J_SETTING_AFTER.id_kotei AND Z_RT_data_J_kotei.id_kikai = J_SETTING_AFTER.id_kikai AND 
--				Z_RT_data_J_kotei.bunban = J_SETTING_AFTER.bunban 	
--				LEFT OUTER JOIN Z_PRTS ON Z_RT_data_J_kotei.id_hinmoku = Z_PRTS.KCODE	
--			WHERE 
--				1=1 
--				AND dbo.Z_RT_data_J_kotei.shift_date LIKE '202208%'
--				AND Z_RT_master_kotei.name_kotei in ('Hariawase',
--													'Double Sheet',
--													'Hariawase',
--													'Hariawase Awal',
--													'Hariawase Polycarbon',
...
--				AND Z_RT_data_J_kotei.flg_sagyokanryo = 1
--				AND dbo.Z_RT_master_sagyosha.flg_opmj = 1
--				--AND Z_RT_data_J_kotei.id_sagyosha1 = 34
--				--AND Z_RT_data_J_kotei.id_hinmoku like '%-S'
--			GROUP BY 	
--				Z_RT_master_sagyosha.id_sagyosha, Z_RT_master_sagyosha.name_sagyosha, Z_RT_master_sagyosha.grp, shift_date, Z_RT_data_J_kotei.id_hinmoku, Z_PRTS.SIYOUW
--		) AS [J_KOTEI]
--		GROUP BY [J_KOTEI].NIK, [J_KOTEI].NAMA, [J_KOTEI].GRP, [J_KOTEI].SHIFT_DATE
...
--	SUM(amnt_OK) + SUM(amnt_NG) + SUM(amnt_PND) AS QTY_INPUT,
--	Z_PRTS.SIYOUW AS CAVITY,
--	(SUM(amnt_OK) + SUM(amnt_NG) + SUM(amnt_PND)) * Z_PRTS.SIYOUW AS SHEET_CAVITY,
--	CASE WHEN (RIGHT(shift_date, 2) = '02') 
--		THEN 
--			CAST(((SUM(amnt_OK) + SUM(amnt_NG) + SUM(amnt_PND)) --QTY INPUT
--			/ NULLIF((((SUM(ttl_sagyo)) - (ISNULL(SUM(J_PAUSE_WORKING.ttl_pause_working),0)) - (ISNULL(SUM(J_SETTING_AFTER.ttl_setting_after), 0))) --WORKING TIME
--			/ 3600 ), 0)
--			* Z_PRTS.SIYOUW) as numeric(36,1)) 
--		ELSE 
--		0 
--	end AS 'HARI_02' 
--FROM 
...
--WHERE 
--	1=1 
--	AND dbo.Z_RT_data_J_kotei.shift_date LIKE '20220802%'
--	AND Z_RT_master_kotei.name_kotei in ('Hariawase',
--										'Double Sheet',
--										'Hariawase',
--										'Hariawase Awal',
...
--=====================================
--OLD QUERY BEFORE * CAVITY 8 JULY 2022
--=====================================
--DECLARE @@QUERY VARCHAR(MAX);
