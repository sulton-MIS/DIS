...
BEGIN TRY
	...
	END ELSE
	SET @@CEK_LAPOR = (SELECT COUNT(1) FROM ad_dis_ma_lapor_asset 
						WHERE no_asset like '%'+ RTRIM(LTRIM(@NO_ASSET)) + '%' AND flg_approval_lapor IS NULL AND created_by like '%'+ RTRIM(LTRIM(@USERNAME)) + '%');
	IF (@@CEK_LAPOR > 0)
	BEGIN

		SET @@CHK = 'NOT APPROVED';
		SET @@ERR = 'Please Contact Admnistrator/ AMS to Approval Data Before !';
